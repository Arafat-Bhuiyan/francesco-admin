...
import { Search, Filter, ChevronDown, Eye, FileText, TrendingUp, DollarSign, Loader2 } from "lucide-react";
import QuotationModal from "./QuotationModal";
import { useQuotationPricingQuery } from "@/redux/features/baseApi"; // Adjust path

const Quotation = () => {
  const { data, isLoading } = useQuotationPricingQuery();
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("All Status");
  const [isModalOpen, setIsModalOpen] = useState(false);
  ...
  const filteredQuotations = useMemo(() => {
    ...
    return quotes.filter((q) => {
      const matchesSearch =
        q.customer.full_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        q.vehicle.toLowerCase().includes(searchTerm.toLowerCase());
      const matchesStatus =
        statusFilter === "All Status" || q.status.toLowerCase() === statusFilter.toLowerCase();
      return matchesSearch && matchesStatus;
    });
  ...
  const stats = [
    ...
    },
    {
      ...
      value: `$${(data?.quotations?.reduce((acc, q) => acc + q.total_amount, 0) || 0).toLocaleString()}`,
      icon: <DollarSign className="w-5 h-5 text-white" />,
      color: "bg-[#F59E0B]",
    },
  ];
  ...
  const getStatusStyle = (status) => {
    switch (status?.toLowerCase()) {
      case "sent": return "bg-blue-50 text-blue-500 border-blue-100";
      case "accepted": return "bg-green-50 text-green-500 border-green-100";
  ...
  return (
    <div className="py-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {stats.map((stat, index) => (
          <div key={index} className="bg-white p-8 rounded-xl border border-gray-100 flex items-center justify-between shadow-sm">
            <div className="space-y-1">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1 group">
          ...
          <input
            ...
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full h-14 pl-14 pr-6 bg-white border border-gray-100 rounded-full text-sm font-semibold focus:outline-none focus:ring-4 focus:ring-blue-50/10 focus:border-blue-500 transition-all shadow-sm"
          />
        </div>
        ...
        <div className="relative min-w-[200px]">
          ...
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value)}
            className="w-full h-14 pl-14 pr-12 bg-white border border-gray-100 rounded-full text-sm font-bold text-gray-700 appearance-none focus:outline-none focus:ring-4 focus:ring-blue-50/10 shadow-sm cursor-pointer"
          >
            <option>All Status</option>
        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100">
                  <th className="py-6 px-10 text-left text-xs font-bold text-gray-400 uppercase">Customer</th>
                  <th className="py-6 px-8 text-left text-xs font-bold text-gray-400 uppercase">Car Details</th>
                  <th className="py-6 px-8 text-center text-xs font-bold text-gray-400 uppercase">Dates</th>
                  <th className="py-6 px-8 text-center text-xs font-bold text-gray-400 uppercase">Total</th>
                  <th className="py-6 px-8 text-center text-xs font-bold text-gray-400 uppercase">Status</th>
                  <th className="py-6 px-10 text-right text-xs font-bold text-gray-400 uppercase">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {filteredQuotations.map((q) => (
                    ...
                    <td className="py-7 px-10">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold">
                          {q.customer.full_name.charAt(0)}
                        </div>
                        <div>
                          <p className="font-bold text-[#111827]">{q.customer.full_name}</p>
                          {q.customer.is_vip && <span className="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded font-black">VIP</span>}
                        </div>
                      </div>
                    </td>
                    <td className="py-7 px-8 font-bold text-gray-700 text-sm">{q.vehicle}</td>
                    <td className="py-7 px-8 text-center text-gray-600 text-sm">{q.rental_dates}</td>
                    <td className="py-7 px-8 text-center font-semibold text-lg">${q.total_amount.toLocaleString()}</td>
                    <td className="py-7 px-8 text-center">
                      <span className={`px-4 py-2 rounded-full text-[10px] font-semibold uppercase border ${getStatusStyle(q.status)}`}>
                        {q.status}
                      </span>
                    </td>
                    <td className="py-7 px-10 text-right">
                      <button
                        ...
                        className="text-blue-600 hover:text-white hover:bg-blue-600 border border-blue-600 px-4 py-2 rounded-xl text-xs font-bold transition-all"
                      >
                        View Details
                      </button>
                    </td>
