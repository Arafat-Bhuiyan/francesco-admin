...
import BookingDetailsModal from "./BookingDetailsModal";
import { useBookingListQuery } from "@/redux/features/baseApi";
    ...
    <div className="py-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1 group">
          ...
          <input
            type="text"
            placeholder="Search by ID, vehicle, or agent..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full h-14 pl-14 pr-6 bg-white border border-gray-100 rounded-full text-sm font-bold focus:outline-none focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 transition-all shadow-sm"
          />
        </div>
        ...
        <div className="relative min-w-[200px]">
          ...
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value)}
            className="w-full h-14 pl-14 pr-12 bg-white border border-gray-100 rounded-full text-sm font-semibold text-gray-700 appearance-none focus:outline-none focus:ring-4 focus:ring-blue-500/5 shadow-sm cursor-pointer"
          >
            <option>All Status</option>
      ...
      <div className="bg-white rounded-md border border-gray-100 overflow-hidden shadow-sm">
        ...
        {isLoading ? (
          <div className="py-20 flex justify-center gap-2"><Loader2 className="w-10 h-10 animate-spin text-blue-500" />Loading...</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="text-[#374151] text-xs font-bold border-b border-gray-50 bg-gray-50/30">
                  <th className="py-6 px-8 text-left text-base">ID</th>
                  <th className="py-6 px-8 text-left text-base">Vehicle</th>
                  <th className="py-6 px-8 text-left text-base">Rental Dates</th>
                  <th className="py-6 px-8 text-center text-base">Status</th>
                  <th className="py-6 px-8 text-left text-base">Agent</th>
                  <th className="py-6 px-8 text-right text-base">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {filteredBookings.map((booking) => (
                  <tr key={booking.id} className="group hover:bg-gray-50/50 transition-colors">
                    <td className="py-6 px-8 text-sm font-medium text-gray-500">#{booking.id}</td>
                    <td className="py-6 px-8 text-sm font-bold text-[#111827]">{booking.vehicle}</td>
                    <td className="py-6 px-8 text-sm font-semibold text-gray-600">{booking.rental_dates}</td>
                    <td className="py-6 px-8 text-center">
                      <span className={`px-4 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-wider ${booking.status === "approved" ? "bg-green-50 text-green-500" :
                    ...
                    <td className="py-6 px-8 text-sm font-semibold text-gray-600">{booking.agent_name}</td>
                    <td className="py-6 px-8 text-right">
                      <button
